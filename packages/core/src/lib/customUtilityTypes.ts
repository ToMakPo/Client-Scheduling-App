 ...
 * }
 * 
 * class Address implements AddressInfo {
 *   ...
 * }
 * 
 * type Original = {
 ...
 * }
 * 
 * type init = Replace<Original, { address: Address | AddressInfo }
 */
export type Replace<T, K extends Partial<Record<keyof T, any>>> = Omit<T, keyof K> & { [P in keyof K]: K[P] };

/** Makes specified keys in the type queryable as single value or array of values.
 * If originally an array, then it allows single value of the array element type as well. */
 ...
 * }
 * 
 * type FlattenedCompany = FlattenChild<CompanyInfo, { address: 'a' }>;
 * ```
 * 
 * Resulting type:
 * ```
